...
class Token final {
  ...
  public:
    Token(
        TokenType type, std::any value, uint64_t start_line, uint64_t start_column, uint64_t end_line,
        uint64_t end_column)
    ...
    bool operator==(const Token &rhs) const {
        return _type == rhs._type && GetValueString() == rhs.GetValueString() && _start_pos == rhs._start_pos
    ...
    std::string                   GetValueString() const {
        ...
        } catch (const std::bad_any_cast &) { DieAndPrint("No suitable cast for token value."); }
        return "Invalid";
    }
