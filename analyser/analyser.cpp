...
std::optional<CompilationError> Analyser::analyseMain() {
    ...
    // <常量声明>
    auto constant_declaration_err = analyseConstantDeclaration();
    if (constant_declaration_err.has_value())
        return constant_declaration_err;

    // <变量声明>
    auto variable_declaration_err = analyseVariableDeclaration();
    if (variable_declaration_err.has_value())
        return variable_declaration_err;

    // <语句序列>
    auto statement_sequence_err = analyseStatementSequence();
    if (statement_sequence_err.has_value())
        return statement_sequence_err;
...
std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
    ...
    while (true) {
        ...
        // 变量可能没有初始化，仍然需要一次预读
        {
            bool initialized = (next.value().GetType() == TokenType::EQUAL_SIGN);
...
std::optional<CompilationError> Analyser::analyseStatementSequence() {
    while (true) {
        ...
        if (!next.has_value())
            return {};
        unreadToken();
        if (next.value().GetType() != TokenType::IDENTIFIER && next.value().GetType() != TokenType::PRINT
...
void Analyser::addUninitializedVariable(const Token &tk) { _add(tk, _uninitialized_vars); }
...
bool Analyser::isInitializedVariable(const std::string &s) { return _vars.find(s) != _vars.end(); }

bool Analyser::isConstant(const std::string &s) { return _consts.find(s) != _consts.end(); }
}  // namespace miniplc0
